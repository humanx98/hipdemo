...
static WwRenderer3DResult __ww_must_check hiprt_triangle_mesh_init(ww_triangle_mesh_ptr self, WwTriangleMeshCreationProperties creation_properties, HipRTRenderContext context);
static void hiprt_triangle_mesh_destroy(ww_triangle_mesh_ptr self);

WwRenderer3DResult hiprt_triangle_mesh_create(HipRTRenderContext context, WwTriangleMeshCreationProperties creation_properties, WwTriangleMesh* triangle_mesh) {
    ...
    WwRenderer3DResult res = hiprt_triangle_mesh_init(self, creation_properties, context);
    if (res.failed) {
...
WwRenderer3DResult hiprt_triangle_mesh_init(ww_triangle_mesh_ptr self, WwTriangleMeshCreationProperties creation_properties, HipRTRenderContext context) {
    *self = {
        .context = context
    };
    ...
    WwRenderer3DResult res = HIP_CHECK(hipMalloc(&self->mesh.triangleIndices, self->mesh.triangleCount * sizeof(int3)));
    if (res.failed) {
        return res;
    }
