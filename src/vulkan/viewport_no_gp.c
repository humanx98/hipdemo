...
VulkanResult vulkan_viewport_no_gp_create_instance(viewport_ptr self, VulkanViewportNoGPCreationProperties creation_properties, const VkDebugUtilsMessengerCreateInfoEXT* debug_create_info) {
    ...
    if (WW_ARRAY_SIZE(validation_layers) > 0) {
        ...
        WW_ARRAY_FOREACH(validation_layers, required_layer) {
            ...
            if (!layer_found) {
                WW_LOG_ERROR("[viewport_ptr] Couldn't find %s vulkan layer\n", *required_layer);
...
VulkanResult vulkan_viewport_no_gp_pick_physical_device(viewport_ptr self, u32 device_index) {
    ...
    if (!queue_family_indices.present.found) {
        WW_LOG_ERROR("[viewport_ptr] Couldn't find present queue family\n");
        res = VULKAN_CHECK(VK_ERROR_INITIALIZATION_FAILED);
        goto failed;
    } else {
        WW_LOG_INFO("[viewport_ptr] present queue family index: %d\n", queue_family_indices.present.index);
    ...
    WW_ARRAY_FOREACH(device_extensions, required_extension) {
        ...
        if (!found_extension) {
            WW_LOG_ERROR("[viewport_ptr] Couldn't find device extension: %s\n", *required_extension);
...
VulkanResult vulkan_viewport_no_gp_get_swap_chain_details(viewport_ptr self) {
    ...
    if (res.failed) {
        ...
        WW_LOG_ERROR("[viewport_ptr] No device surface formats");
        res = VULKAN_CHECK(VK_ERROR_INITIALIZATION_FAILED);
        goto failed;
    }
    ...
    if (res.failed) {
        ...
        WW_LOG_ERROR("[viewport_ptr] No device surface present modes");
        res = VULKAN_CHECK(VK_ERROR_INITIALIZATION_FAILED);
        goto failed;
    }
    ...
    if (!found_format) {
        WW_LOG_WARN("[viewport_ptr] Couldn't find wanted surface fomat");
...
VKAPI_ATTR VkBool32 VKAPI_CALL vulkan_debug_callback(VkDebugUtilsMessageSeverityFlagBitsEXT severity,
                                              ...
                                              void* user_data) {
    #define debug_callback_log_msg "[viewport_ptr] validation layer%s: %s\n"
    ...
    switch (severity) {
        ...
        case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
            ...
            break;
        default:
            WW_LOG_ERROR("[viewport_ptr] validation layer%s and uknown msg severity: %s\n", msg_type_str, callback_data->pMessage); 
            break;
    }
